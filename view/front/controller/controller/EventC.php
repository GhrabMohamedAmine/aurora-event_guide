...
class EventController {
    ...
    private function addEvent() {
        ...
        $data['id_user'] = $_SESSION['user_id']; // Définir id_user à partir de la session
    ...
    private function editEvent() {
        ...
        $data = $this->sanitizeInput($_POST, $required);
        $data['id_user'] = $_SESSION['user_id']; // Mettre à jour id_user si nécessaire
    ...
    private function listEvents() {
        $events = $this->afficherEvenements();
        $this->jsonResponse(true, '', $events);
    }

    public function afficherEvenements() {
        $db = getDB();
        $sql = "SELECT e.*, u.prenom AS user_name 
                FROM evenement e 
        ...
        try {
            ...
            error_log("Erreur dans afficherEvenements: " . $e->getMessage());
            return [];
        }
    }
